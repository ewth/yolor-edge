...
if [[ -z "${IN_DOCKER}" || !${IN_DOCKER} ]]; then
    echo "Please run from inside the docker container only."
    exit
fi
...
WANDB_PROJECT="jetson-yolor" WANDB_ENTITY="ewth" SHM_SIZE=${SHM_SIZE} python3 /yolor/test.py \
