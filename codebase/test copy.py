...
def test(data,
    ...
    if training:  # called by train.py
        ...
        # @todo 12/12: this is looking into how models are loaded, not functional code 
        # vars = ['module_defs', 'module_list', 'modules', 'type', 'version', 'yolo_layers', 'named_children', 'named_modules', 'named_parameters', 'parameters']
        vars = ['module_defs', 'version', 'yolo_layers']

        for x in vars:
            print(f"\n\n{x}:\n\n")
            if not callable(getattr(model, x)):
                print(getattr(model, x))
            else:
                print(f"(skipping callable {x})\n\n")
        ...
        print("\n\n\n\n")
        print(model.info)
        # @todo 12/12: remove quit after model info determined
        print("Bailing here after model loaded")
        quit()
        ...
        # @todo 12/12: Why is this failing?
        ...
        #except:
            ...
            # @todo 12/12: if the torhch.load etc above fails, the below results in another error:
    ...
    # @todo 12/12: remove quit after model info determined
    print("Bailing here after all model loading finished")
    quit()
    ...
    if plots and wandb:
        ...
        if save_dir.joinpath('precision-recall_curve.png').exists:
            try:
